...
class Component extends Disposable(Object) {
  ...
  /**
   * Calls render() and inserts the result into the given parent element. Also attaches a disposer
   * so that the parent DOM element owns this Component.
   */
  _mount(elem) {
 ...
 *
 *       dom('div', _insert_(new Comp1()), _insert_(new Comp2())
 *
...
function createElem(elem, ComponentClass, ...args) {
  new ComponentClass(...args)._mount(elem);
}
