...
/**
 * Make the given elment own the disposable, and call its dispose method when dom.dispose() is
...
function autoDisposeElem(elem, disposable) {
  _domdispose.onDisposeElem(elem, () => disposable.dispose());
}
...
function _subscribe(elem, valueObs, callback) {
  autoDisposeElem(elem, binding.subscribe(valueObs, callback));
}
