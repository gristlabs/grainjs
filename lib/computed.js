...
class Computed extends observable.Observable {
  ...
  constructor(read, write, dependencies) {
    ...
    this._depListeners = fast.map(this._dependencies, obs => this._subscribeTo(obs));
  ...
  /**
   * Reevaluates the computed and saves the result as its current value. This is intended for
   * internal use. A computed that is properly subscribed to dependencies should always be
   * up-to-date, so if you find yourself call this method, you are very likely doing something
   * wrong or hacky.
   */
  recompute() {
  ...
  /**
   * Schedule this computed for recomputation. It will get recomputed after all its dependencies
   * have been brought up-to-date.
   */
  _enqueueRecompute() {
    computed_queue.enqueue(this);
  }
   ...
   */
  _updateSubs() {
    // TODO: The updates to _priority may cause it to grow without bounds. We also need tests that
    // it doesn't change unnecessarily. Might it be better to only update it when deps change, and
    // to propagate the update to subscribers? Or too complicated?
    let priority = 0;
    for (let d of this._dependencies) {
      if (d._priority > priority) {
        priority = d._priority;
      }
    }
    this._dynDeps.forEach((listener, obs) => {
      if (listener.inUse) {
        listener.inUse = false;
        if (obs._priority > priority) {
          priority = obs._priority;
        }
      } else {
   ...
   */
  _subscribeTo(obs) {
    return obs.onChange.addListener(this._enqueueRecompute, this);
  }
 ...
 */
let bundleChanges = computed_queue.bundleChanges;
...
module.exports.bundleChanges = bundleChanges;
